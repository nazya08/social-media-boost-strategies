...
export const ConfigSchema = z.object({
  ...
  runtime: z.object({
    ...
    partsTargetMin: z.number().int().min(2).max(15).default(6),
    partsTargetMax: z.number().int().min(2).max(15).default(8),
...
export const loadConfig = (): AppConfig => {
  ...
  const raw = {
    ...
    runtime: {
      ...
      partsTargetMin: intFromEnv(process.env.PARTS_TARGET_MIN, 6),
      partsTargetMax: intFromEnv(process.env.PARTS_TARGET_MAX, 8),
